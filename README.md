...
## Clip如何通过对比学习对齐图像和文本空间

![image-20250706000129501](img/image-20250706000129501.png)

CLIP是由 OpenAI 提出的一种强大的多模态预训练框架，旨在通过对大规模图文对数据进行对比学习，构建统一的图像与自然语言语义空间。传统的图像分类模型通常依赖于人为标注的类别标签（如 ImageNet 上的“cat”、“dog”等），标签数量有限且语义粒度受限，难以泛化到开放域任务。CLIP 的提出则打破了这一限制，它使用自然语言描述作为监督信号，从开放网络中自动收集的大规模图文对数据中学习视觉与语言之间的对应关系，无需特定任务的数据集标注，具备良好的零样本迁移能力。

CLIP 的结构为典型的双塔（Dual-Encoder）架构：一侧是图像编码器，常用的包括 ResNet 和 Vision Transformer（ViT）；另一侧是文本编码器，通常是 Transformer 类型的语言模型。输入图像经过图像编码器处理后，输出一个固定长度的视觉向量；文本经过文本编码器后也被映射为相同维度的语言向量。模型的核心优化目标是将图像和其对应的文本描述在向量空间中拉近距离，同时将不匹配的图文对拉远距离。具体训练时，CLIP 从一个 batch 中采样多个图文对，对所有图像和所有文本分别编码后计算两两点积形成相似度矩阵。这个相似度矩阵的每个元素表示某张图像与某段文本之间的语义接近程度。CLIP 使用 InfoNCE（softmax + cross-entropy）损失函数，从两个方向进行监督：一是让图像预测对应文本（行方向），二是让文本预测对应图像（列方向），训练目标是让对角线位置的匹配对具有最大的相似度，从而在整个矩阵上构成一个多分类任务。这个设计充分利用了 batch 内图文对的负样本结构，在每个 batch 中都形成了 $N^2$ 个对比项，提高了训练效率和语义对齐能力。

CLIP 在图像编码器上采用了 Vision Transformer（ViT）后性能大幅提升。ViT 能够通过全局注意力机制建模图像中不同区域的长期依赖关系，更好地捕捉图像中的高级语义内容，相比传统 CNN 对图像细节和语义分布的建模更为灵活，因而在大规模图文配对任务中展现出显著优势。CLIP 使用的训练数据集非常庞大，包含约 4 亿对图文配对数据，来源广泛，语义丰富。这种规模和多样性为模型提供了强大的开放域泛化能力，尤其在零样本学习任务中效果显著。通过简单地为分类任务编写自然语言 prompt，例如“a photo of a cat”，CLIP 就能直接基于余弦相似度在多个候选类别中选出最匹配的文本，从而实现无需下游训练的图像分类。

## Clip不是已经和文本对齐了吗？为什么不能直接送入LLM？

尽管 CLIP 将图像和文本成功映射到同一个共享语义空间，但这种对齐并不意味着图像特征可以直接输入到大语言模型中进行生成任务。这是因为 CLIP 的图像输出通常是一个固定长度的向量，它没有 token 序列结构，也不包含位置编码，而语言模型是典型的自回归结构，要求输入是 token embedding 序列，具有上下文顺序与语法结构。因此，CLIP 的输出无法直接用于 LLM 的文本生成，这也是后来如 BLIP-2、LLaVA 等多模态大模型引入“桥接模块”的原因。这些模块包括 Q-Former、MLP 映射层或 learnable visual tokens，其作用是将 CLIP 的图像 embedding 映射为符合语言模型输入要求的 token embedding 序列，实现结构和语义的双重兼容，最终才能实现图像到语言生成的能力。

# Blip

## Blip1

BLIP1没有直接对齐大语言模型（LLM），它的设计目的是在不依赖大规模语言模型的情况下，构建一个多模态预训练框架，同时具备图文匹配、图像描述和图文检索等能力。相比于此前的 CLIP 或 ALIGN 等模型仅专注于图文对齐任务，BLIP 强调构建更全面的视觉-语言表征，通过多任务训练提升模型对多模态任务的适应能力。

![image-20250706002801661](img/image-20250706002801661.png)

BLIP1 使用的是一个三模块结构：

1. **视觉编码器**：通常为 ViT（Vision Transformer），可能从 CLIP 等模型中初始化。
2. **文本编码器/解码器**：使用 BERT-like 结构，用于文本理解或生成。
3. **图文交叉编码器（cross-attention）**：用于建模图像与文本之间的深层交互。

它支持以下三种任务的统一建模：

- **图文匹配（ITM）**：判断图像和文本是否匹配
- **图文对齐（ITC）**：使用 InfoNCE 对比损失让图像和文本嵌入接近
- **图像文本生成（Captioning）**：通过语言建模目标生成描述性文本

BLIP 的整体架构由三个关键模块组成：图像编码器、文本编码器，以及一个可选的图文交叉注意力模块。图像编码器通常采用预训练的 ViT，用于提取视觉特征；文本编码器则为 BERT-like 结构，用于对输入的自然语言进行编码。为了支持图像描述生成任务，BLIP 同时具备一个语言解码器头部，可实现基于图像的文本生成。此外，在图文匹配和图文对齐任务中，BLIP 引入了一个图文交叉编码器，采用 Transformer 架构进一步对图像和文本进行融合建模。整体上，BLIP 可灵活配置为三种模式：纯编码（用于图文对齐），交叉编码（用于图文匹配），编码-解码（用于图像描述生成）。

训练目标方面，BLIP 采用多任务联合学习策略，具体包括图文对齐（ ITC）、图文匹配（TM）和图像描述生成（Captioning）。在图文对齐任务中，模型通过 InfoNCE 损失最大化匹配图文对的相似度，同时压低非配对图文之间的相似度；图文匹配任务则是一个二分类任务，用于判断图像和文本是否配对；图像描述任务则利用自回归的语言模型训练方式，根据图像生成描述性文本。一个关键的创新是，BLIP 引入了**伪标签对生成机制**，即通过图文对齐模型生成伪文本，辅助训练图文匹配与描述任务。这种自监督机制有效提升了弱监督图文数据的利用效率，显著提高了模型性能。

BLIP v1 为后续多模态大模型的发展打下了坚实基础。它不仅在多模态检索、图文匹配、VQA 等任务中实现了强大的性能，还展示了如何构建一个通用的、多功能的预训练框架，并首次验证了在统一结构下三类多模态任务协同训练的可行性。通过充分利用网络图文数据，并加入伪标签与多任务学习策略，BLIP 显著降低了对高质量标注数据的依赖，成为当前视觉语言表示学习中广泛使用的基线模型之一。

## Blip2

![image-20250706094328519](img/image-20250706094328519.png)

BLIP-2 是一类“桥梁式”的多模态学习框架，旨在**高效连接视觉感知模块和强大的大语言模型（LLM）**。其目标是提升图像到文本生成的效果，并大幅减少参数量与训练成本。相比 BLIP-1 强调融合式图文建模，BLIP-2 采用了**模块解耦的 two-stage 策略**，允许冻结大型语言模型，仅训练中间连接模块，从而大幅提升了效率与通用性。

BLIP-2 架构由三个主要部分组成：

1. **视觉编码器（Vision Encoder）**：如 ViT-G/14，负责从图像中提取高维 patch token 表征，通常是预训练的。
2. **Q-Former（Query Transformer）**：这是 BLIP-2 的关键组件。它输入固定数量的 learnable query token，通过 cross-attention 与视觉特征交互，从中提取少量对语言相关的**压缩视觉 token**。
3. **语言模型（LLM）**：如 Flan-T5、OPT、LLaMA。它是冻结的、强大的语言模型，用于生成自然语言输出。

BLIP-2 的设计中，**LLM 从未直接处理原始图像 token**。图像经过 Vision Encoder 得到视觉特征，再由 Q-Former 将这些特征转换为 **定长的、语言风格的 query embedding**。这些 query 被拼接为语言模型的输入前缀，从而指导 LLM 的生成。因此，LLM 实际“看到”的是图像经过 Q-Former 解码后的伪语言 token embedding，而不是空间感强、维度高的视觉 patch token。这种“视觉语言化”的设计是 BLIP-2 的关键创新，使得 frozen LLM 能顺畅地接受图像信息。

# Llava

![image-20250706100424060](img/image-20250706100424060.png)

LLaVA（Large Language and Vision Assistant）是一个轻量而高效的多模态大模型架构，其最核心的设计理念在于使用**极简的多模态对齐方式**来实现视觉信息与大语言模型的融合。具体而言，LLaVA 并未引入复杂的中间模块如 Q-Former 或 Adapter，而是采用**一层线性映射层**，将图像编码器提取出的视觉特征直接投射到与 LLaMA 兼容的嵌入空间中。这种“视觉 → 语言 token embedding”的简单桥接方式，尽管形式极简，却在下游的多模态对话与理解任务中展现出出色的性能，证明了 CLIP 表征与 LLM 的潜在语义兼容性。

LLaVA 的训练分为两个阶段。第一阶段是**特征对齐预训练**，模型从 CC3M 中筛选出 59.5 万图文对，并将其转化为单轮指令数据，构建“图像 + 问题 → caption”格式的训练样本。在此阶段，视觉编码器和 LLaMA 模型均保持冻结，仅训练图像到语言嵌入空间的投影层，使图像特征能被语言模型有效接收，可视作构建“视觉 tokenizer”。第二阶段为**端到端指令微调**，继续训练投影层，并解冻 LLaMA，增强模型的对话和推理能力。训练数据包括三类任务：图像描述、多轮问答、复杂推理，其中一部分由 GPT 生成以扩充样本。在如 ScienceQA 等任务中，模型需结合图文上下文，生成推理过程并选择正确答案，从而全面提升多模态理解与生成能力。

为解决多模态训练中常见的数据匮乏问题，LLaVA 还提出了一种**高质量数据自生成策略**。该策略并不依赖已有的多模态问答数据，而是借助**非多模态版本的 GPT（如 GPT-3.5）**，结合精心设计的 prompt 模板，对大规模图像描述文本进行问答式重写与扩展。这样，模型构建者成功获得了包含**图像描述、用户对话和复杂推理**三类任务的大量图文问答训练样本，为 LLaVA 的多模态能力打下了坚实基础，同时大大节省了人工数据标注成本。
